...
| 4. LSP + DAP + Syntax | ✅ Complete | Full LSP (rename, format, outline, breadcrumbs, peek), DAP (watches, exceptions), 17-language tree-sitter |
...
- [ ] **In-code `@ai-caution: [architecture-debt]` landmine markers not yet cross-referenced here** (hygiene gap, found 2026-07-29): three markers exist in code but weren't listed in this section per CLAUDE.md's own tagging convention (these are invariant/landmine warnings, not file-size debt, so they don't belong in `mae-audit.md`'s size-ceiling "Known exceptions" list — CLAUDE.md scopes that list explicitly to "files over the audit's size ceilings"). `shared/kb/src/cozo_store/schema.rs:85,95` — two `DbInstance::new` panic sources (sled `PageCache::start` on a broken store directory; a real, timing-dependent CI-observed sqlite bootstrap-transaction `SQLITE_BUSY` unwrap) caught via `catch_unwind` so a broken on-disk KB degrades to a normal open failure instead of crashing the editor at startup. `shared/kb/src/federation.rs:676` — `IngestMode::Incremental` skips re-parsing (and re-stamping newly-added per-node metadata) for any file whose content hash hasn't changed since the last import, so a node persisted by an older ingestion-logic version stays stale until a `Full` reimport runs; tracked inline via issue #323. `shared/kb/src/cozo_store/db.rs:5` — the `pub(super)` script-execution helpers (`run_mut*`/`run_immut*`/`now_epoch`/`node_put_params`) are called from ~12 sites across every sibling module in the directory; signatures must stay stable.
