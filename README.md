...
7. **(Optional) Pair with VS Code + Copilot** — install the ["MAE for VS
   Code"](https://github.com/cuttlefisch/mae-vscode) extension (its own repo, independent
   release cadence) from the Marketplace or a built `.vsix`; it auto-spawns a headless MAE
   instance per workspace and pairs it with Copilot's Agent mode over MCP, zero manual
   `.vscode/mcp.json` setup. See
