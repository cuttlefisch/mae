...
The actual export/render logic is compiled Rust, in-tree
(`crates/export/src/html_graph.rs`, wired in via `mae-ai`'s
`kb_export_html.rs`; briefly spiked as a standalone `bilingual-kb-export`
sibling project during development — see its
`kb/adrs/0002-mae-module-not-scheme-reimplementation.org` for the
Rust-vs-Scheme rationale — folded back in-tree so this ships as a normal,
self-contained module with no external path-dependency). This module is
thin wiring on top of it, matching `kb-graph-view`'s shape: no export logic
lives in Scheme.
