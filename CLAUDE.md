...
16. **Controls that bound the agent are not part of the peer surface.** Principle #3 makes the AI a peer for *doing work*. It does not extend to the controls that decide **what the agent is allowed to do** — permission tiers, workspace trust, KB membership and residency. Those must be reachable by the human and **not** by the agent, because a control the agent can change is not a control. Concretely, and each of these is a deliberate, evidenced exception rather than an oversight:
    ...
    - **The agent does not choose its own standing instructions.** `ai_guidance_kb` names the corpus of practices every AI session is told to follow, and `initialize.instructions` is rebuilt per MCP connection from the *live* option value — so a `Write`-tier agent could blank or repoint it for every subsequent session in the process. Persisting was already blocked, but an in-memory change lasting the life of the editor is not meaningfully weaker. Now Privileged. The human keeps a first-class path (`:set-save`, and the deterministic `mae --ensure-guidance-config`); only the agent's own reach is removed. Its siblings stay ordinary: `ai_guidance_inline_budget_chars` caps how much is inlined and `ai_guidance_export_live_sync` mirrors it to a file — neither decides *which* corpus applies.
    - **A system KB is not the agent's to manage.** MAE's own corpora (`mae_kb::system_kb`) are name-reserved against `kb_register` and refuse `kb_share`/`kb_share_p2p`/`kb_reimport`/`kb_unregister`. Sharing one was the sharp edge: `NodeSource` is absent from the CRDT wire payload, so nodes arrive at the peer re-stamped `Federation`, stripping the `Seed` provenance that is the only thing making shipped content read-only.
