...
/// (`viewports`, `rendered`, `render_epoch`, `last_theme_name`) MUST be pruned in
...
pub struct GraphView {
    ...
    /// Per-window theme name `rendered[win_id]` was last flattened against
    /// — live-testing bug: cycling the theme (`SPC t t`) sets `Editor.theme`
    /// but the graph view's `rendered` cache (unlike a text buffer, which
    /// re-derives colors fresh every paint) is only ever regenerated by an
    /// explicit `Editor::graph_view_reflatten_window` call, and nothing
    /// triggered that on a theme change — the stale cache kept drawing the
    /// OLD theme's colors until some unrelated event (a click) happened to
    /// trigger a reflatten. `Editor::sync_open_graph_viewports` (the same
    /// self-healing per-tick resync that already catches a stale viewport
    /// pixel size) compares this against `Editor.theme.name` and reflattens
    /// on a mismatch, regardless of *how* the theme changed. Per-`WindowId`
    /// (like `viewports`/`rendered`/`render_epoch` above), NOT a single
    /// buffer-wide value — a reflatten stamps only the ONE window it just
    /// rebuilt, so a second window on the same buffer that hasn't been
    /// reflattened yet must still read as stale on its own account. Absent
    /// entry for a window that hasn't been reflattened yet.
    pub(crate) last_theme_name: HashMap<WindowId, String>,
}
...
mod tests {
    ...
    fn flatten_scene_graph_cached_skips_edges_to_a_hidden_tier_node() {
        ...
        let render_tiers = vec![RenderTier::Full, RenderTier::Hidden];
        let diagram_indices = vec![0usize, 0usize];
        let mut cache = None;
        let elements = flatten_scene_graph_cached(
            &scene,
            &viewport,
    ...
    fn flatten_scene_graph_cached_skips_edges_to_a_clustered_tier_node() {
        ...
        let viewport = mae_canvas::scene::Viewport::default();
        let degrees = vec![0u32, 0u32];
        let render_tiers = vec![RenderTier::Full, RenderTier::Clustered];
        let diagram_indices = vec![0usize, 0usize];
        let mut cache = None;
        let elements = flatten_scene_graph_cached(
            &scene,
    ...
    fn flatten_scene_graph_cached_still_draws_edges_between_two_full_tier_nodes() {
        ...
        let viewport = mae_canvas::scene::Viewport::default();
        let degrees = vec![0u32, 0u32];
        let render_tiers = vec![RenderTier::Full, RenderTier::Full];
        let diagram_indices = vec![0usize, 0usize];
        let mut cache = None;
        let elements = flatten_scene_graph_cached(
            &scene,
        ...
        assert_eq!(
            line_count, 1,
            "an edge between two Full-tier nodes must still draw"
        );
    }
    ...
    fn flatten_scene_graph_cached_boundary_self_loop_stub_disappears_when_its_source_is_hidden() {
        ...
        let render_tiers = vec![RenderTier::Hidden];
        let diagram_indices = vec![0usize];
        let mut cache = None;
        let elements = flatten_scene_graph_cached(
            &scene,
            &viewport,
