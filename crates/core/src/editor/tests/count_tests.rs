...
fn count_prefix_default_none() {
    ...
    assert_eq!(editor.vi.count_prefix, None);
}
...
fn move_down_count_consistent_from_row_zero() {
    ...
    editor.vi.count_prefix = Some(17);
    editor.dispatch_builtin("move-down");
...
fn move_up_with_count_clamps() {
    ...
    editor.vi.count_prefix = Some(10); // more than available
...
fn g_with_count_goes_to_line() {
    let mut editor = editor_with_text("line1\nline2\nline3\nline4\nline5");
    editor.vi.count_prefix = Some(3); // 3G = go to line 3 (1-indexed = row 2)
    editor.dispatch_builtin("move-to-last-line");
    assert_eq!(editor.window_mgr.focused_window().cursor_row, 2);
}
...
fn g_with_count_clamps() {
    let mut editor = editor_with_text("line1\nline2\nline3");
    editor.vi.count_prefix = Some(100); // beyond buffer
    editor.dispatch_builtin("move-to-last-line");
...
fn gg_with_count() {
    ...
    editor.vi.count_prefix = Some(2); // 2gg = go to line 2 (1-indexed = row 1)
...
fn paste_after_with_count() {
    ...
    editor.vi.registers.insert('"', "x".to_string());
    editor.vi.count_prefix = Some(3);
