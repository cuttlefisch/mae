...
/// `crates/mae/src/bootstrap.rs`) and making `sled` permanently unreachable
/// regardless of what the user configured.
#[test]
fn kb_storage_engine_option_round_trip() {
    let mut editor = Editor::new();
    assert_eq!(editor.kb.storage_engine, "sqlite");
    assert_eq!(editor.get_option("kb_storage_engine").unwrap().0, "sqlite");

    editor.set_option("kb_storage_engine", "sled").unwrap();
    assert_eq!(editor.kb.storage_engine, "sled");
    assert_eq!(editor.get_option("kb_storage_engine").unwrap().0, "sled");

    let err = editor.set_option("kb_storage_engine", "bogus");
    assert!(
        err.is_err(),
        "an unrecognized storage engine must be rejected"
    );
    assert_eq!(
        editor.kb.storage_engine, "sled",
        "a rejected set must not mutate the field"
    );
}
