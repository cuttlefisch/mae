...
fn fold_end_at_basic() {
    let mut editor = Editor::new();
    editor.buffers[0].insert_text_at(0, "a\nb\nc\nd\ne\n");
    editor.buffers[0].folded_ranges.push((1, 4));
    assert_eq!(editor.buffers[0].fold_end_at(1), Some(4));
    assert_eq!(editor.buffers[0].fold_end_at(0), None);
    assert_eq!(editor.buffers[0].fold_end_at(2), None);
}

#[test]
fn code_block_cache_populated_after_set() {
    let mut editor = Editor::new();
    editor.buffers[0].insert_text_at(0, "```rust\nfn main() {}\n```\n");
    editor.buffers[0].set_file_path(std::path::PathBuf::from("test.md"));
    editor
        .syntax
        .set_language(0, crate::syntax::Language::Markdown);
    let flavor = editor.effective_markup_flavor(0);
    let gen = editor.buffers[0].generation;
    let lines = crate::detect_code_block_lines(&editor.buffers[0], flavor);
    editor.code_block_cache.insert(
        ...
        crate::syntax::ViewportCodeBlockCache {
            ...
            flavor,
            line_start: 0,
            line_end: editor.buffers[0].line_count(),
            lines: lines.clone(),
        },
    );
    let cached = editor.code_block_cache.get(&0).unwrap();
...
fn viewport_local_code_blocks_match_full_buffer() {
    ...
    let full = crate::detect_code_block_lines(&editor.buffers[0], flavor);
    // Viewport-local for middle range (lines 2..7).
    let local = crate::detect_code_block_lines_for_range(&editor.buffers[0], flavor, 2, 7);
...
fn viewport_local_code_blocks_backward_scan() {
    ...
    let local = crate::detect_code_block_lines_for_range(&editor.buffers[0], flavor, 2, 4);
