...
impl Editor {
    ...
    pub fn execute_command(&mut self, cmd: &str) -> bool {
        ...
        match command {
            ...
            "kb-save" | "kb-load" => {
                self.set_status("KB persistence is now automatic via CozoDB. Use :kb-register / :kb-reimport for external KBs, or :kb-set-scope (SPC n S) to switch which already-registered KB you're working in.");
                true
            }
            "theme" | "set-theme" => {
