...
///
/// Future (ADR-006): Collaborative buffers will gain a `sync_doc: Option<Arc<yrs::Doc>>`
/// field. Local edits generate yrs transactions; remote changes rebuild the rope
/// via the `mae-sync` bridge. The ropey rope remains the rendering source.
pub struct Buffer {
    rope: Rope,
    ...
    /// Collaborative document address. Determines save policy and identity
    /// for cross-session CRDT stability. Set on share or join.
    pub doc_address: Option<mae_sync::DocAddress>,
    /// Collaborative doc_id used for remote update routing. Set during share/join.
    /// Buffer names may differ from doc_ids (e.g. buffer "main.rs" vs doc_id
    /// "file:abc123/src/main.rs"), so we store the doc_id explicitly.
    pub collab_doc_id: Option<String>,
    /// True when the buffer's collab connection was lost but CRDT state is preserved.
    /// Local edits accumulate; on reconnect, resync merges them with server state.
    pub collab_offline: bool,
    /// True when this client is the sharer (authoritative saver) for this collab doc.
    /// Joiners have this set to `false` and save locally only (no `save_intent` broadcast).
    pub collab_is_sharer: bool,
    /// Collaborative sync document. When Some, edits generate yrs updates for broadcast.
    pub sync_doc: Option<mae_sync::text::TextSync>,
    /// Pending sync updates generated by local edits (drained by MCP broadcaster).
    ///
    /// Expected bounds: ~10 entries max between 100ms drain ticks (10 inserts ×
    /// ~50 bytes each ≈ 500 bytes). A warning fires if this exceeds 1000 entries,
    /// which indicates the drain loop is stalled or disconnected.
    pub pending_sync_updates: Vec<Vec<u8>>,
}
...
impl Buffer {
    pub fn new() -> Self {
        Buffer {
            ...
            doc_address: None,
            collab_doc_id: None,
            collab_offline: false,
            collab_is_sharer: false,
            sync_doc: None,
            pending_sync_updates: Vec::new(),
        }
    }
    ...
    /// Apply a remote sync update (from another client).
    /// Returns `Err` if sync is not enabled on this buffer.
    pub fn apply_sync_update(&mut self, update: &[u8]) -> Result<(), mae_sync::SyncError> {
    ...
    fn sync_insert(&mut self, char_offset: usize, text: &str) {
        if let Some(sync) = &mut self.sync_doc {
            ...
            if self.pending_sync_updates.len() > 1000 {
                tracing::warn!(
                    buffer = %self.name,
                    pending = self.pending_sync_updates.len(),
                    "pending_sync_updates exceeds 1000 — drain may be stalled"
                );
            }
        }
    }
    ...
    ///
    /// Uses `reconcile_to()` to compute minimal insert/delete operations via
    /// character-level diff, preserving CRDT vector clocks and tombstones.
    /// This is safe for multi-client undo — peers receive only the delta,
    /// not a full-state replacement.
    ...
    pub fn insert_char(&mut self, win: &mut Window, ch: char) {
        ...
        self.sync_insert(pos, &ch.to_string());
    ...
    pub fn delete_line(&mut self, win: &mut Window) -> String {
        ...
        self.sync_delete(line_start, line_chars);
    ...
    pub fn delete_word_backward(&mut self, win: &mut Window) {
        ...
        self.sync_delete(pos, cursor - pos);
    ...
    pub fn delete_to_line_start(&mut self, win: &mut Window) {
        ...
        self.sync_delete(line_start, cursor - line_start);
        self.push_undo(EditAction::DeleteRange {
            pos: line_start,
            text: deleted,
        });
    ...
    pub fn delete_to_line_end(&mut self, win: &mut Window) {
        ...
        self.sync_delete(cursor, line_end - cursor);
        self.push_undo(EditAction::DeleteRange {
            pos: cursor,
            text: deleted,
        });
    ...
    pub fn delete_range(&mut self, start: usize, end: usize) {
        ...
        self.sync_delete(start, end - start);
...
mod tests {
    ...
    fn hash_detects_external_edit() {
        ...
        assert!(buf.check_disk_changed_by_hash());
    }

    #[test]
    fn hash_stable_on_no_change() {
        let tmp = tempfile::TempDir::new().unwrap();
        let file = tmp.path().join("hash_test.txt");
    ...
    #[test]
    fn hash_handles_missing_file() {
        let tmp = tempfile::TempDir::new().unwrap();
        let file = tmp.path().join("hash_test.txt");
        std::fs::write(&file, "delete me").unwrap();
        let buf = Buffer::from_file(&file).unwrap();
        std::fs::remove_file(&file).unwrap();
        // check_disk_changed_by_hash returns false when file is missing (can't read)
        assert!(!buf.check_disk_changed_by_hash());
    }

    #[test]
    fn hash_handles_empty_file() {
        let tmp = tempfile::TempDir::new().unwrap();
    ...
    #[test]
    fn hash_length_always_64() {
        // SHA-256 hex digest is always 64 chars
        let inputs: Vec<&str> = vec!["", "hello", "日本語"];
        for input in &inputs {
            let tmp = tempfile::TempDir::new().unwrap();
            let file = tmp.path().join("hash_len.txt");
            std::fs::write(&file, input).unwrap();
            let buf = Buffer::from_file(&file).unwrap();
            assert_eq!(buf.content_hash.as_ref().unwrap().len(), 64);
        }
        // Also test a large input
        let tmp = tempfile::TempDir::new().unwrap();
        let file = tmp.path().join("hash_len_large.txt");
        std::fs::write(&file, "a".repeat(10000)).unwrap();
        let buf = Buffer::from_file(&file).unwrap();
        assert_eq!(buf.content_hash.as_ref().unwrap().len(), 64);
    }

    // --- Collaborative sync tests ---

    #[test]
    fn enable_sync_populates_doc() {
        let mut buf = Buffer::new();
        buf.rope = Rope::from_str("hello world");
        buf.enable_sync(42);
        assert!(buf.sync_doc.is_some());
        assert_eq!(buf.sync_doc.as_ref().unwrap().content(), "hello world");
    }

    #[test]
    fn insert_char_generates_sync_update() {
        let (mut buf, mut win) = new_buf_win();
        buf.enable_sync(1);
        buf.insert_char(&mut win, 'A');
        assert_eq!(buf.pending_sync_updates.len(), 1);
        assert_eq!(buf.text(), "A");
        // Sync doc should match rope
        assert_eq!(buf.sync_doc.as_ref().unwrap().content(), "A");
    }

    #[test]
    fn delete_generates_sync_update() {
        let mut buf = Buffer::new();
        buf.rope = Rope::from_str("hello");
        buf.enable_sync(1);
        buf.delete_range(1, 3); // delete "el"
        assert_eq!(buf.pending_sync_updates.len(), 1);
        assert_eq!(buf.text(), "hlo");
        assert_eq!(buf.sync_doc.as_ref().unwrap().content(), "hlo");
    }

    #[test]
    fn apply_sync_update_rebuilds_rope() {
        // A starts with content
        let mut buf_a = Buffer::new();
        buf_a.rope = Rope::from_str("hello");
        buf_a.enable_sync(1);

        // B starts empty, receives A's full state
        ...
        // A inserts, B applies the update
        buf_a.insert_text_at(5, " world");
    ...
    #[test]
    fn no_sync_updates_when_disabled() {
        let (mut buf, mut win) = new_buf_win();
        // sync_doc is None by default
        buf.insert_char(&mut win, 'X');
        assert!(buf.pending_sync_updates.is_empty());
    }

    #[test]
    fn reload_from_disk_rebuilds_sync() {
        let tmp = tempfile::TempDir::new().unwrap();
        let file = tmp.path().join("sync_reload.txt");
        std::fs::write(&file, "original").unwrap();
        let mut buf = Buffer::from_file(&file).unwrap();
        buf.enable_sync(1);
        assert_eq!(buf.sync_doc.as_ref().unwrap().content(), "original");
        // Change file on disk and reload
        std::fs::write(&file, "reloaded content").unwrap();
        buf.reload_from_disk().unwrap();
        assert_eq!(buf.text(), "reloaded content");
        assert_eq!(buf.sync_doc.as_ref().unwrap().content(), "reloaded content");
    }

    #[test]
    fn replace_contents_rebuilds_sync() {
        let mut buf = Buffer::new();
        buf.rope = Rope::from_str("before");
        buf.enable_sync(1);
        assert_eq!(buf.sync_doc.as_ref().unwrap().content(), "before");
        buf.replace_contents("after replacement");
        assert_eq!(buf.text(), "after replacement");
        assert_eq!(
    ...
    #[test]
    fn undo_rebuilds_sync() {
        let (mut buf, mut win) = new_buf_win();
        buf.enable_sync(1);
        buf.insert_text_at(0, "hello");
        assert_eq!(buf.sync_doc.as_ref().unwrap().content(), "hello");
        buf.undo(&mut win);
        assert_eq!(buf.text(), "");
    ...
    #[test]
    fn buffer_close_drops_sync() {
        let mut buf = Buffer::new();
        buf.rope = Rope::from_str("synced content");
        buf.enable_sync(1);
        assert!(buf.sync_doc.is_some());
        // Simulate close by dropping — just verify disable_sync works cleanly
        let state = buf.disable_sync();
        assert!(state.is_some());
        assert!(buf.sync_doc.is_none());
        // No panic = success
