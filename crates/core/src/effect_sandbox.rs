...
pub fn external_effects_blocked_for(caller_is_test_build: bool) -> bool {
    if !(caller_is_test_build || env_blocks_effects()) {
        return false;
    }
...
/// [`effect_sandbox::with_external_effects`](crate::effect_sandbox::with_external_effects).
#[macro_export]
macro_rules! external_effects_blocked {
    () => {
        $crate::effect_sandbox::external_effects_blocked_for(cfg!(test))
    };
}
