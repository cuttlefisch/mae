//! Unit tests for the executor module (extracted from mod.rs to keep the
//! source under the file-size ceiling — idiomatic sibling `#[path]` test module).
...
// --- Submodule split verification tests ---

#[test]
fn executor_submodule_permission_format() {
    use crate::tools::PermissionPolicy;
    let policy = PermissionPolicy::default();
    let output = super::permission::format_permissions_info(&policy);
    assert!(output.contains("auto-approve tier"));
}
