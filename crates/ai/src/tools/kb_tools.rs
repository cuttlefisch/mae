...
pub(super) fn kb_tool_definitions() -> Vec<ToolDefinition> {
    vec![
        ...
        .prop(
            "chord_config",
            "object",
            "Optional per-call overrides for the exported page's chord-diagram layout/timing constants (bilingual_kb_export::html_graph::ChordDiagramConfig). Accepted numeric keys: hover_growth_factor, stroke_buffer_px, cosmetic_cushion_px, min_onscreen_radius_px, initial_pad_px, edge_pull_back, wedge_gap_radians, history_depth_cap, wedge_corner_radius_fraction, search_debounce_ms, ui_transition_ms. Any key omitted falls back to its kb_export_* editor option (set-option!-able, e.g. `(set-option! \"kb-export-hover-growth-factor\" \"2.5\")`), which itself defaults to the same hardcoded values this tool always used before this option existed.",
        )
        .required(["id", "path"])
        .permission(PermissionTier::Write)
        .build(),
