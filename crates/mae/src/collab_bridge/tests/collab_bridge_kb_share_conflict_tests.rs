//! ADR-105 D4/D5: what happens when a minted collab id is REFUSED.
//!
//! Split from `collab_bridge_kb_identity_tests.rs` to stay under the structural
//! ceiling. The identity tests there cover minting and resolution; these cover
//! recovery — which is a separate property, and the one with a dangerous
//! direction: re-minting a KB that HAS a confirmed share destroys its signed
//! membership (finding A), so half of these assert that recovery is refused.
