...
MAE's own tool permissions are separate from agent approval. Use the ~ai_permissions~ tool to see the current tier and where it came from. Set it with ~(set-option! "ai-tier" "shell")~ in =init.scm=, or ~MAE_AI_PERMISSIONS~ for a single launch.
