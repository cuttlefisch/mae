...
#+filetags: :tutorial:collaboration:daemon:sync:

** Setting Up Collaborative Editing

This lesson [[concept:collaborative-state?rel=teaches][teaches collaborative state]] and the [[concept:sync-engine?rel=teaches][sync engine]]. It walks you through enabling real-time collaborative editing in MAE, from installing the daemon to sharing your first buffer with a peer.
...
brew services info mae  # check status
#+end_src

*** macOS: launchd (manual)

#+begin_src bash
cp assets/com.cuttlefisch.mae-daemon.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.cuttlefisch.mae-daemon.plist
launchctl list com.cuttlefisch.mae-daemon  # check status
#+end_src

To stop: =launchctl unload ~/Library/LaunchAgents/com.cuttlefisch.mae-daemon.plist=

*** macOS: Firewall

If the macOS Application Firewall is enabled (System Settings > Network > Firewall), allow =mae-daemon= when prompted, or manually add it in Firewall Options.

*** Client-Frame Workflow

Connect a running editor with ~:collab-connect~ (=SPC C c=), after setting ~collab-server-address~:
#+begin_example
