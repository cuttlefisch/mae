...
Run `mae-daemon` on the same machine to coordinate multiple MAE instances or AI agents on the same project.
...
|-----|---------|-------------|
| `SPC C s` | `:collab-start-server` | Start a local daemon |
...
- `mae doctor` (CLI) — checks daemon process, port binding, WAL integrity

See also: [[concept:collab-architecture?rel=references][Collaborative Editing Architecture]], [[lesson:collab-setup]], [[concept:sync-engine]], [[index]]
