...
Prerequisites: [[requires:lesson:debugging][Lesson 11: Debugging]]

MAE has built-in tools for diagnosing issues and understanding editor behavior.

*** Watchdog
The [[teaches:concept:watchdog][watchdog]] monitors the event loop for stalls. If the main thread stops responding for >2 seconds, it dumps thread backtraces to the log.
  ~MAE_LOG=mae=trace mae~ — enable watchdog logging
  The watchdog runs automatically; no user action needed.

*** Event recording
[[teaches:concept:event-recording][Event recording]] captures every input event and command dispatch for replay and bug reporting.
...
*** Introspect
The [[teaches:concept:introspect][introspect]] AI tool provides a diagnostic snapshot of the editor's internal state: threads, performance counters, lock contention, buffers, shell processes, and AI session info.
