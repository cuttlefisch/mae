...
| Function | Description |
|----------|-------------|
| `(json-encode value)` | Encode a Scheme value as a JSON string (objects: alist of `(key . value)` string-keyed pairs; arrays: vectors `#(...)`; JSON `null` round-trips through the symbol `null`) |
| `(json-decode str)` | Decode a JSON string into a Scheme value (inverse mapping) |

```scheme
(json-decode "{\"name\":\"mae\",\"tags\":[\"editor\",\"scheme\"]}")
;; => (("name" . "mae") ("tags" . #("editor" "scheme")))

(cdr (assoc "name" (json-decode "{\"name\":\"mae\"}")))  ;; => "mae"
```

### File I/O

R7RS `open-output-file` / `write-string` / `close-output-port` write files directly and
synchronously — unlike `(write-file ...)` below, this is real, general-purpose I/O:

```scheme
(define port (open-output-file "/tmp/report.txt"))
(write-string "Generated by mae\n" port)
(close-output-port port)
```

`(open-input-file path)` / `read-line` / `close-input-port` is the read-side equivalent.

> **`write-file` is test-harness-only, not a general API.** `(write-file PATH CONTENT)`
> only queues onto an internal buffer that mae's `--test` runner drains after each test
> step — outside `mae --test`, calling it is a silent no-op (nothing is ever written).
> Use `open-output-file` + `write-string` + `close-output-port` above for real file I/O
> from modules or scripts.
...
| `(mae json)` | JSON encode/decode: `json-encode`, `json-decode` |
