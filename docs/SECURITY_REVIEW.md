...
| **I2** | Identity | **No key rotation / rebind** — rotating a key = a new principal → lose every KB membership + every content-key wrap | WEAKNESS | **ADR-040 (Proposed)** — cross-signed `Rebind` op (old key endorses new, history-preserving); planned rotation only, compromise recovery = owner-evict + ADR-039 block + re-join (self-service deferred) |
